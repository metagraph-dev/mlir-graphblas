""" This wraps https://github.com/llvm/llvm-project/blob/main/mlir/lib/ExecutionEngine/SparseTensorUtils.cpp """
...
cdef extern from "mlir/ExecutionEngine/SparseTensorUtils.h" nogil:
    ctypedef enum DimLevelType:
        kDense "DimLevelType::kDense"
        kCompressed "DimLevelType::kCompressed"
        kSingleton "DimLevelType::kSingleton"

cdef extern from "SparseTensorUtils.cpp" nogil:
    ...
    cdef cppclass SparseTensorStorage[P, I, V]:
        ...
        SparseTensorStorage(const vector[uint64_t], const uint64_t *, const DimLevelType *, SparseTensorCOO[V] *) except +
        uint64_t getRank() const
...
cpdef empty_mlir_sparse_tensor_safe(uint64_t[:] sizes, uint8_t[:] sparsity, uint64_t[:] perm=None, pointer_type=np.uint64, index_type=np.uint64, value_type=np.float64):
    ...
    cdef DimLevelType *sparsity_array = <DimLevelType*>malloc(sizeof(uint8_t) * D)
    for i in range(D):
        sparsity_array[i] = DimLevelType.kCompressed if sparsity[i] else DimLevelType.kDense
        ...
        if pointer_type_num == np.NPY_UINT8:
            ...
            if index_type_num == np.NPY_UINT64:
                if value_type_num == np.NPY_FLOAT32:
                    ...
                    data = (new SparseTensorStorage[uint64_t, uint64_t, int8_t](sizes_vector, perm_ptr, sparsity_array, tensor_int8))
                elif value_type_num == np.NPY_INT16:
                    data = (new SparseTensorStorage[uint64_t, uint64_t, int16_t](sizes_vector, perm_ptr, sparsity_array, tensor_int16))
                elif value_type_num == np.NPY_INT32:
                    data = (new SparseTensorStorage[uint64_t, uint64_t, int32_t](sizes_vector, perm_ptr, sparsity_array, tensor_int32))
                elif value_type_num == np.NPY_INT64:
                    data = (new SparseTensorStorage[uint64_t, uint64_t, int64_t](sizes_vector, perm_ptr, sparsity_array, tensor_int64))
                else:
                    raise TypeError(f"Invalid type combo for pointers, indices, and values: {pointer_dtype}, {index_dtype}, {value_dtype}")
            elif index_type_num == np.NPY_UINT32:
                if value_type_num == np.NPY_FLOAT32:
