...
class DebugResult:
    ...
    def explore(self):
        ...
        sequential = pn.Column(
            ...
            pn.Row(
                pn.pane.HTML(
                    code_block.replace("{{textarea}}", self.stages[0]).replace(
                        "{{id}}", "foobar_seq1"
                    )
                ),
                pn.pane.HTML(
                    code_block.replace("{{textarea}}", self.stages[1]).replace(
                        "{{id}}", "foobar_seq2"
                    )
                ),
                max_width=400,
        ...
        sequential[0].link(
            sequential[2][0], callbacks={"value": partial(code_callback, offset=-1)}
        )
